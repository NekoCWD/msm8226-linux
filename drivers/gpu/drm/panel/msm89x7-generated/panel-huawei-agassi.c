...
static int huawei_agassi_panel_on(struct huawei_agassi_panel *ctx)
{
...
static int huawei_agassi_panel_off(struct huawei_agassi_panel *ctx)
{
	return 0;
}

static int huawei_agassi_panel_prepare(struct drm_panel *panel)
{
	...
	ret = huawei_agassi_panel_on(ctx);
	if (ret < 0) {
...
static int huawei_agassi_panel_unprepare(struct drm_panel *panel)
{
	struct huawei_agassi_panel *ctx = to_huawei_agassi_panel(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;
	...
	ret = huawei_agassi_panel_off(ctx);
	if (ret < 0)
...
static const struct drm_panel_funcs huawei_agassi_panel_panel_funcs = {
	.prepare = huawei_agassi_panel_prepare,
	.unprepare = huawei_agassi_panel_unprepare,
